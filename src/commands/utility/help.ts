import { ActionRow, ActionRowBuilder, ButtonBuilder, ButtonStyle, ChatInputCommandInteraction, ComponentType, EmbedBuilder, Message, SlashCommandBuilder } from "discord.js";
import fs from 'fs'
...
module.exports = {
	...
	async execute(interaction: ChatInputCommandInteraction | Message, profileData: any, inputs : {[key: string]: any}) {
        const commandName = inputs ? inputs["command"] : (interaction as ChatInputCommandInteraction).options.getString('command')
        ...
        if(commandName) {
            ...
            interaction.reply(`That command was not found.`)
            return
        } else {
